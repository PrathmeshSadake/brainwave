...
function QuizForm({}: {}) {
  ...
  const onSubmit = async (data: Input) => {
    ...
    const { data: adaptive_quiz, error } = await supabase
      .from("adaptive_quiz")
      .insert({
        userId: session?.user?.id,
        topic: data.topic,
        total_questions: data.amount,
      })
      .select();
    ...
    if (adaptive_quiz && adaptive_quiz.length > 0) {
      router.push(`/adaptive_quiz/${adaptive_quiz[0].id}`);
    }
  };
  form.watch();
